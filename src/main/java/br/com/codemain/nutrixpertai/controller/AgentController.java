...
public class AgentController {
        ...
        @ApiResponses(value = {
                        @ApiResponse(responseCode = "200", description = "Resposta do agente processada com sucesso", content = @Content(mediaType = "application/json", schema = @Schema(implementation = RunAgentResponseDto.class))),
                        @ApiResponse(responseCode = "400", description = "Requisição inválida (ex: campos faltando)", content = @Content),
                        @ApiResponse(responseCode = "503", description = "Serviço do agente (Python) indisponível ou demorou demais", content = @Content)
        })
        ...
        @Operation(summary = "Criar um novo feedback", description = "Cria um novo feedback do usuário para uma resposta específica do agente.")
        @ApiResponses(value = {
                        @ApiResponse(responseCode = "200", description = "Feedback registrado com sucesso", content = @Content(mediaType = "application/json", schema = @Schema(implementation = FeedbackResponseDto.class))),
        ...
        @Operation(summary = "Retorna todos os feedbacks", description = "Retorna todos os feedbacks do usuário para uma conversa específica com o agente.")
        @ApiResponses(value = {
                        @ApiResponse(responseCode = "200", description = "Feedbacks retornados com sucesso", content = @Content(mediaType = "application/json", schema = @Schema(implementation = FeedbackResponseDto.class))),
                        @ApiResponse(responseCode = "400", description = "Requisição inválida", content = @Content),
                        @ApiResponse(responseCode = "422", description = "Erro de validação (ex: campos obrigatórios faltando)", content = @Content)
        })
        ...
        @Operation(summary = "Buscar mensagens da sessão", description = "Retorna todos os detalhes e o histórico de mensagens de uma sessão específica do usuário.")
        @ApiResponses(value = {
                        @ApiResponse(responseCode = "200", description = "Sessão encontrada e mensagens retornadas", content = @Content(mediaType = "application/json", schema = @Schema(implementation = SessionInfoResponseDto.class) // O
                                                                                                                                                                                                                                        // DTO
                                                                                                                                                                                                                                        // de
                                                                                                                                                                                                                                        // resposta
                        )),
                        @ApiResponse(responseCode = "404", description = "Sessão ou usuário não encontrado", content = @Content)
        })
        @GetMapping("/sessions/{userId}/{sessionId}")
        public ResponseEntity<SessionInfoResponseDto> getSessionMessages(
                        @Parameter(description = "ID do usuário", required = true, example = "1") @PathVariable String userId,

                        @Parameter(description = "ID da sessão", required = true, example = "1") @PathVariable String sessionId) {
                SessionInfoResponseDto response = agentService.getSessionMessages(userId, sessionId);
                return ResponseEntity.ok(response);
        }

        @Operation(summary = "Listar sessões do usuário", description = "Retorna uma lista de todas as sessões de um usuário específico, com a primeira mensagem.")
        @ApiResponses(value = {
                        @ApiResponse(responseCode = "200", description = "Lista de sessões retornada com sucesso", content = @Content(mediaType = "application/json", array = @ArraySchema(schema = @Schema(implementation = SessionListItemDto.class)))),
